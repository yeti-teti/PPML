...
class RDPAccountant:
    ...
    def init(self, noise_multiplier, target_epsilon, target_delta, sample_rate):
...
class PrivacyEngine:
    ...
    def init(self, model, batch_size, max_grad_norm, noise_multiplier):
        self.model = model
    ...
    def add_noise(self):
        ...
        for p in parameters:
            noise = torch.randn_like(p.grad) * noisescale
            p.grad.add(noise)
